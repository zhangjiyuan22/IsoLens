...
def getEventInfo(eventname,cfgfile=False):
    ...
    if eventinfo.use_piE==True:
        eventinfo.piEEmean,eventinfo.piEEerr = [float(pi) for pi in p.get('Weight Options','piEEvalue').split()]
        eventinfo.piENmean,eventinfo.piENerr = [float(pi) for pi in p.get('Weight Options','piENvalue').split()]
        eventinfo.piEcor = float( p.get('Weight Options','piEcor') )
    ...
    eventinfo.sDisk_min = float(p.get('Galactic Component','sDisk_min'))
    eventinfo.sDisk_max = float(p.get('Galactic Component','sDisk_max'))
